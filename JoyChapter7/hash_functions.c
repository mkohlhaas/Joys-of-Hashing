...
uint32_t
tabulation_hash_r8_q8(uint32_t x, uint8_t* T)
{
...
uint32_t
tabulation_hash_r8_q16(uint32_t x, uint8_t* T)
{
  // these are all known at compile time
  const int      r       = 8;
  const uint32_t no_cols = 1 << r;
...
uint32_t
tabulation_hash_r8_q32(uint32_t x, uint8_t* T)
{
  // these are all known at compile time
  const int      r       = 8;
  const uint32_t no_cols = 1 << r;
...
uint32_t
tabulation_hash_r4_q8(uint32_t x, uint8_t* T)
{
  // these are all known at compile time
  const int      r       = 4;
  const uint32_t no_cols = 1 << r;
...
uint32_t
tabulation_hash_r4_q16(uint32_t x, uint8_t* T)
{
  // these are all known at compile time
  const int      r       = 4;
  const uint32_t no_cols = 1 << r;
...
uint32_t
tabulation_hash_r4_q32(uint32_t x, uint8_t* T)
{
  // these are all known at compile time
  const int      r       = 4;
  const uint32_t no_cols = 1 << r;
...
uint32_t
tabulation_hash_r2_q8(uint32_t x, uint8_t* T)
{
  // these are all known at compile time
  const int      r       = 2;
  const uint32_t no_cols = 1 << r;
  ...
  y ^= T_[7 * no_cols + (x & mask)];
  x >>= r;
  y ^= T_[8 * no_cols + (x & mask)];
  x >>= r;
  y ^= T_[9 * no_cols + (x & mask)];
  x >>= r;
  y ^= T_[10 * no_cols + (x & mask)];
  x >>= r;
  y ^= T_[11 * no_cols + (x & mask)];
  x >>= r;
  y ^= T_[12 * no_cols + (x & mask)];
  x >>= r;
  y ^= T_[13 * no_cols + (x & mask)];
  x >>= r;
  y ^= T_[14 * no_cols + (x & mask)];
  x >>= r;
  y ^= T_[15 * no_cols + (x & mask)];
  x >>= r;
  return y;
}

uint32_t
tabulation_hash_r2_q16(uint32_t x, uint8_t* T)
{
  // these are all known at compile time
  const int      r       = 2;
  const uint32_t no_cols = 1 << r;
...
uint32_t
tabulation_hash_r2_q32(uint32_t x, uint8_t* T)
{
  // these are all known at compile time
  const int      r       = 2;
  const uint32_t no_cols = 1 << r;
...
uint32_t*
poly_build_table(int k)
{
  return malloc(sizeof(uint32_t) * k);
}

void
poly_sample(uint32_t* a, int k, uint32_t p)
{
  for (int i = 0; i < k; ++i)
    a[i] = rand() % p;
}
...
uint32_t
poly_hash2(uint32_t x, uint32_t* a, uint32_t p)
{
...
uint32_t
poly_hash5(uint32_t x, uint32_t* a, uint32_t p)
{
  ...
  // Since all values fit in 32 bits we can add them in 64 bits without overflow
  uint64_t y = a0 + ax1 + ax2 + ax3 + ax4;
  return (uint32_t)(y % p);
}

uint64_t
mod_Mersenne(uint64_t x, uint8_t s)
{
...
uint32_t
poly_hash2_Mersenne(uint32_t x, uint32_t* a, uint8_t s)
{
...
uint32_t
poly_hash5_Mersenne(uint32_t x, uint32_t* a, uint8_t s)
{
  // No need for % p for the first value, it will fit.
  uint64_t x1 = (uint64_t)x;
